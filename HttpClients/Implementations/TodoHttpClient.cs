...
public class TodoHttpClient : ITodoService
{
    ...
    {
        HttpResponseMessage response = await client.PostAsJsonAsync("/controller", dto);
    ...
    public async Task<ICollection<Todo>> GetAsync(string? userName, int? userId, bool? completedStatus, string? titleContains)
    {
        ...
        HttpResponseMessage response = await client.GetAsync("/controller"+query);
        string content = await response.Content.ReadAsStringAsync();
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception(content);
    ...
    public async Task UpdateAsync(TodoUpdateDto dto)
    {
        ...
        HttpResponseMessage response = await client.PatchAsync("/controller", body);
        if (!response.IsSuccessStatusCode)
        {
            string content = await response.Content.ReadAsStringAsync();
            throw new Exception(content);
    ...
    {
        HttpResponseMessage response = await client.GetAsync($"/controller/{id}");
        string content = await response.Content.ReadAsStringAsync();
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception(content);
    ...
    {
        HttpResponseMessage response = await client.DeleteAsync($"controller/{id}");
        if (!response.IsSuccessStatusCode)
        {
            string content = await response.Content.ReadAsStringAsync();
            throw new Exception(content);
